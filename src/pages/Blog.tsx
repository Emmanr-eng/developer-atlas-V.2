    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-4">
          <h1 className="text-4xl font-bold tracking-tighter text-slate-900">Insights</h1>
              ...
              <div className="flex flex-col h-full">
                <div className="flex items-center space-x-3 mb-6">
                  {post.tags.slice(0, 2).map(tag => (
                    <span key={tag} className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 backdrop-blur-sm">
                      {tag}
                    </span>
                  ))}
                  <div className="h-3 w-px bg-black/8" />
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                    {formatDate(post.createdAt)}
                  </span>
                </div>

                <h2 className="text-2xl font-bold tracking-tight mb-4 group-hover:text-emerald-600 transition-colors leading-tight text-slate-900">
                  <Link to={`/blog/${post.id}`}>{post.title}</Link>
                </h2>
                
                <p className="text-sm text-slate-500 mb-8 grow leading-relaxed line-clamp-2">
                  {post.summary}
                </p>

                <div className="flex items-center justify-between pt-6 border-t border-black/6 mt-auto">
                  <div className="flex items-center space-x-2">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-[10px] text-slate-600 border border-black/8"
                  ...
                  <Link
                    to={`/blog/${post.id}`}
                    className="inline-flex items-center space-x-2 text-[10px] font-black uppercase tracking-widest text-emerald-600 hover:text-emerald-500"
                  >
                    Read &rarr;
                  </Link>
                </div>
      ...
      {filteredPosts.length === 0 && (
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          >
            ...
            <Rss className="w-16 h-16 text-emerald-600/20 mx-auto mb-6 animate-pulse" />
            <h3 className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter mb-4">No Insights Found</h3>
            <p className="text-slate-500 text-sm max-w-md mx-auto mb-8 font-medium">Your search query did not return any architectural nodes. Try one of our suggested topics below.</p>
            
            <button className="bg-emerald-600 text-white px-8 py-3 font-black text-[10px] uppercase tracking-[0.2em] hover:scale-105 transition-transform shadow-[0_0_20px_rgba(16,185,129,0.15)]" style={{ borderRadius: '20px' }}>
              Request an Article
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-6">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Suggested Topics</h4>
              <div className="grid grid-cols-2 gap-3">
                {['React', 'Node.js', 'Junior Experience', 'Microservices', 'TypeScript', 'Serverless'].map(topic => (
                  <button 
                    key={topic} 
                    onClick={() => setSearch(topic)}
                    className="p-4 text-left glass-btn hover:border-emerald-600/30 transition-all group"
                    style={{ borderRadius: '20px' }}
                  >
                    <span className="text-xs font-bold text-slate-500 group-hover:text-emerald-600">{topic}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Trending Insights</h4>
              <div className="space-y-4">
                {posts.slice(0, 3).map((post, i) => (
                  ...
                  >
                    <span className="text-2xl font-black text-slate-200 group-hover:text-emerald-600/20 transition-colors uppercase italic">0{i+1}</span>
                    <div className="grow">
                      <h5 className="text-[11px] font-black text-slate-600 uppercase italic tracking-tight group-hover:text-slate-900 transition-colors">{post.title}</h5>
                    </div>
